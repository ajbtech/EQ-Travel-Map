...
def draw_zone_path(my_zone_list, output_path=None):
    ...
    # Linking trapezoids first, then every disc on top, so the bands sit beneath
    # the zone circles. Within each pass newest events are drawn first so older,
    # lower-level routes remain visible on top.
    for draw_event in reversed(draw_events):
        draw_event.draw_trapezoid(map_renderer)
    for draw_event in reversed(draw_events):
